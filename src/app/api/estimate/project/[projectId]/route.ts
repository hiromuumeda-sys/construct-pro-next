...
import { buildEstimatePDF } from "~/server/pdf/document";
...
export async function GET(
  ...
  const pdfBuffer = await buildEstimatePDF(project, orderRows);
  const disposition =
    req.nextUrl.searchParams.get("inline") === "1" ? "inline" : "attachment";
  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `${disposition}; filename="estimate-${project.id}.pdf"`,
    },
  });
}
